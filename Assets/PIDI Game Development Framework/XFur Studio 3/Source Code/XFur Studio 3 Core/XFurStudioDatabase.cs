...
namespace XFurStudio3.Core {
    ...
    [CreateAssetMenu(fileName = "XFur Studio 3 Database", menuName = "XFur Studio 3/New Database Asset")]
    public class XFurStudioDatabase : ScriptableObject {

        private string databaseVersion = "v3.0.0";

        public string Version { get { return databaseVersion; } }
                
        public enum XFurRenderingMode { Standard, Universal, HighDefinition }

        public bool receiveURPShadows = true;
        ...
        public void UpdateShadows() {
            ...
            if ( !receiveURPShadows ) {
                URPShells.shaderKeywords = new string[] { "_RECEIVE_SHADOWS_OFF" };
                URPShellsDouble.shaderKeywords = URPShells.shaderKeywords;
            }
            else {
                URPShells.shaderKeywords = new string[0];
                URPShellsDouble.shaderKeywords = URPShells.shaderKeywords;
            }
        ...
        public void LoadResources() {
            ...
            if ( !VFXProgressive ) {
                VFXProgressive = Shader.Find( "Hidden/XFur Studio 3/VFX/VFXProgressive" );
            }

            if ( !GPUPhysics ) {
                GPUPhysics = Shader.Find( "Hidden/XFur Studio 3/Physics/GPU Physics" );
            }            

            if ( !PainterBase ) {
                PainterBase = Shader.Find( "Hidden/XFur Studio 3/Designer/Painter" );
            }            

            if ( !PainterFill ) {
                PainterFill = Shader.Find( "Hidden/XFur Studio 3/Designer/Filler" );
            }

            if ( !PainterUnwrap ) {
                PainterUnwrap = Shader.Find( "Hidden/XFur Studio 3/Designer/Auto Unwrap" );
            }

            if ( !XFShells ) {
                XFShells = Shader.Find( "Hidden/XFur Studio 3/XF Shells" );
            }


            if ( !Basic4 ) {
                Basic4 = Shader.Find( "Hidden/XFur Studio 3/Basic Shells 4" );
            }
            
            if ( !Basic8 ) {
                Basic8 = Shader.Find( "Hidden/XFur Studio 3/Basic Shells 8" );
            }


            if ( !XFHDRP ) {
                XFHDRP = Shader.Find( "Hidden/XFur Studio 3/HDRP Shells" );
            }

            if ( !XFHDRPEmissive ) {
                XFHDRPEmissive = Shader.Find( "Hidden/XFur Studio 3/HDRP Shells Emissive" );
            }
            ...
            if( !HDRPEmissiveDouble ) {
                if ( XFHDRP ) {
                    HDRPEmissiveDouble = new Material( Shader.Find( "Hidden/XFur Studio 3/HDRP Shells EmissiveD" ) );
                ...
                HDRPEmissiveDouble.enableInstancing = true;
                HDRPEmissiveDouble.shader = Shader.Find( "Hidden/XFur Studio 3/HDRP Shells EmissiveD" );
                HDRPEmissiveDouble.SetFloat( "_DoubleSidedEnable", 1 );
                HDRPEmissiveDouble.SetFloat( "_DoubleSidedNormalMode", 1 );
            }
            ...
            if ( !XFURP ) {
                XFURP = Shader.Find( "Hidden/XFur Studio 3/URP Shells" );
            }
            ...
            if ( !URPShellsDouble ) {
                if ( XFURP ) {
                    ...
                    URPShellsDouble.shader = Shader.Find( "Hidden/XFur Studio 3/URP ShellsD" );
        ...
        public void UpdateNormalmapUse() {

            if ( XFShellF ) {
                if ( UseNormalmaps )
                    XFShellF.EnableKeyword( "USE_NORMALMAP" );
                else
                    XFShellF.DisableKeyword( "USE_NORMALMAP" );
            }
            
            if ( XFShellI ) {
                if ( UseNormalmaps )
                    XFShellI.EnableKeyword( "USE_NORMALMAP" );
                else
                    XFShellI.DisableKeyword( "USE_NORMALMAP" );
            }
            
            if ( XFShellDoubleI ) {
                if ( UseNormalmaps )
                    XFShellDoubleI.EnableKeyword( "USE_NORMALMAP" );
                else
                    XFShellDoubleI.DisableKeyword( "USE_NORMALMAP" );
            }
            
            if ( XFShellDoubleF ) {
                if ( UseNormalmaps )
                    XFShellDoubleF.EnableKeyword( "USE_NORMALMAP" );
                else
                    XFShellDoubleF.DisableKeyword( "USE_NORMALMAP" );
            }

            if ( URPShells ) {
                if ( UseNormalmaps )
                    URPShells.EnableKeyword( "USE_NORMALMAP" );
                else
                    URPShells.DisableKeyword( "USE_NORMALMAP" );
            }

            if ( URPShellsDouble ) {
                if ( UseNormalmaps )
                    URPShellsDouble.EnableKeyword( "USE_NORMALMAP" );
                else
                    URPShellsDouble.DisableKeyword( "USE_NORMALMAP" );
            }

            if ( HDRPShells ) {
                if ( UseNormalmaps )
                    HDRPShells.EnableKeyword( "USE_NORMALMAP" );
                else
                    HDRPShells.DisableKeyword( "USE_NORMALMAP" );
            }

            if ( HDRPShellsDouble ) {
                if ( UseNormalmaps )
                    HDRPShellsDouble.EnableKeyword( "USE_NORMALMAP" );
                else
                    HDRPShellsDouble.DisableKeyword( "USE_NORMALMAP" );
            }
            
            if ( HDRPEmit ) {
                if ( UseNormalmaps )
                    HDRPEmit.EnableKeyword( "USE_NORMALMAP" );
                else
                    HDRPEmit.DisableKeyword( "USE_NORMALMAP" );
            }

            if ( HDRPEmitDouble ) {
                if ( UseNormalmaps )
                    HDRPEmitDouble.EnableKeyword( "USE_NORMALMAP" );
                else
                    HDRPEmitDouble.DisableKeyword( "USE_NORMALMAP" );
            }


            if ( Basic4Mat ) {
                if ( UseNormalmaps )
                    Basic4Mat.EnableKeyword( "USE_NORMALMAP" );
                else
                    Basic4Mat.DisableKeyword( "USE_NORMALMAP" );
            }

            if ( Basic8Mat ) {
                if ( UseNormalmaps )
                    Basic8Mat.EnableKeyword( "USE_NORMALMAP" );
                else
                    Basic8Mat.DisableKeyword( "USE_NORMALMAP" );
            }
