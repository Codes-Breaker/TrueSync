...
namespace XFurStudio3.Core {
    ...
    public static class XFurStudioAPI {
        ...
        public static void LoadPaintResources() {
            if ( !PainterMaterial ) {
                PainterMaterial = new Material( Shader.Find( "Hidden/XFur Studio 3/Designer/Painter" ) );
            }

            if ( !FillerMaterial ) {
                FillerMaterial = new Material( Shader.Find( "Hidden/XFur Studio 3/Designer/Filler" ) );
            }

            if ( !UnwrapMaterial ) {
                UnwrapMaterial = new Material( Shader.Find( "Hidden/XFur Studio 3/Designer/Auto Unwrap" ) );
            }
                ...
                case PaintDataMode.FurMask:
                    ...
                    if ( target.FurDataProfiles[matIndex].FurData0 ) {
                        if ( target.FurDataProfiles[matIndex].FurData0.GetType() != typeof( RenderTexture ) ) {
                            ...
                            Graphics.Blit( target.FurDataProfiles[matIndex].FurData0, sourceTex );
